"""
API routes module (Chinese version).
...
def start_collection():
    try:
        ...
        if success:
            return jsonify({'success': True, 'message': '采集已启动'})
        return jsonify({'success': False, 'message': '采集器启动失败'})
...
def funding_start():
    try:
        ...
        return jsonify({'success': True, 'message': '资费率监控已启动'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)})
...
def list_files():
    ...
    try:
        p = Path(folder)
        if not p.exists():
            return jsonify({'success': False, 'message': f'文件夹不存在: {folder}'})
        if not p.is_dir():
            return jsonify({'success': False, 'message': f'路径不是文件夹: {folder}'})
        files = []
...
def load_file():
    ...
    fp = Path(path)
    if not fp.exists():
        return jsonify({'success': False, 'message': f'文件不存在: {path}'})
    try:
        records = []
...
def download_files_batch():
    ...
    if not valid_files:
        return jsonify({'success': False, 'message': '没有可下载的有效文件'}), 400
