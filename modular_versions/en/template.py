...
<head>
    ...
    <title>Cryptocurrency millisecond level K-line chart system</title>
...
<body>
<div class="container">
    <h1>📊 Cryptocurrency Millisecond Level K-Line Chart System Pro</h1>
    <div class="subtitle">Real-time collection · Tariff rate monitoring · Automatic window collection · Historical file viewing | Millisecond-level accuracy</div>

    <div class="tab-nav">
        <button class="tab-btn active" onclick="switchTab('live')">🔴 Real-time collection</button>
        <button class="tab-btn" onclick="switchTab('funding')">💰 Automatic collection of tariff rates</button>
        <button class="tab-btn" onclick="switchTab('history')">📂 Historical files</button>
    </div>

    <!-- ══════════ TAB: 实时采集 ══════════ -->
    <div id="tab-live" class="tab-content active">
        <div class="config-panel">
            <div class="config-title">⚙️ Data collection configuration</div>
            ...
            <div class="button-group">
                <button class="btn-success" onclick="startCollection()" id="startBtn">▶️ Start collecting</button>
                <button class="btn-danger"  onclick="stopCollection()"  id="stopBtn" disabled>⏹️ Stop</button>
                <button class="btn-info"    onclick="downloadData()">💾 Download</button>
                <button onclick="clearData()">🗑️ Clear</button>
            </div>
        </div>
        <div id="alertBox"></div>
        <div class="status-bar">
            <div class="status-item"><div class="status-label">state</div><div class="status-value status-stopped" id="collectionStatus">Stopped</div></div>
            <div class="status-item"><div class="status-label">exchange</div><div class="status-value" id="currentExchange">--</div></div>
            <div class="status-item"><div class="status-label">trading pair</div><div class="status-value" id="currentSymbol">--</div></div>
            <div class="status-item"><div class="status-label">Number of data items</div><div class="status-value" id="dataCount">0</div></div>
            <div class="status-item"><div class="status-label">latest price</div><div class="status-value" id="latestPrice">--</div></div>
            <div class="status-item"><div class="status-label">Number of K lines</div><div class="status-value" id="candleCount">0</div></div>
        </div>
        <div class="chart-controls">
            <div class="config-grid">
                <div class="control-group">
                    <label>📈K line period (milliseconds)</label>
                ...
                <div class="control-group">
                    <label>📊 Display the number of K lines</label>
                    <input type="number" id="maxCandles" value="100" min="20" max="1000" step="10">
                </div>
                <div class="control-group">
                    <label>🔄 Refresh interval</label>
                    <select id="refreshInterval">
                        <option value="1">1 second</option><option value="2" selected>2 seconds</option>
                        <option value="5">5 seconds</option>
                    </select>
                </div>
                <div class="control-group">
                    <label>&nbsp;</label>
                    <button onclick="toggleAutoRefresh()" id="autoRefreshBtn">▶️Auto refresh</button>
                </div>
            </div>
        </div>
        <div id="chart"></div>
    ...
    <div id="tab-funding" class="tab-content">
        ...
        <!-- 监控配置 -->
        <div class="config-panel">
            <div class="config-title">💰 Tariff rate monitoring & automatic collection configuration</div>
            <div class="config-grid">
                <div class="control-group">
                    <label>🏆 Display TOP N currencies</label>
                    <input type="number" id="topN" value="5" min="1" max="20">
                </div>
                <div class="control-group">
                    <label>⏰ Collection starts X minutes before settlement</label>
                    <input type="number" id="preMinutes" value="10" min="1" max="60">
                </div>
                <div class="control-group">
                    <label>⏰ Stop collecting X minutes after settlement</label>
                ...
                <div class="control-group">
                    <label>🔥 Minimum 24h transaction volume (100 million U)<span style="color:#e67e22;font-size:10px">Filter very low flow</span></label>
                    <input type="number" id="minVolumeUsd" value="0.3" min="0" step="0.1"
                           title="低于此24h成交额的币种不纳入采集。0=完全不过滤。建议0.3亿以上保证最基本流动性">
                    <small style="color:#888;font-size:10px;margin-top:2px">0=No filtering | 0.3=30 million U (recommended) | 5=500 million U (mainstream currency)</small>
                </div>
                <div class="control-group">
                ...
                <div class="control-group">
                    <label>⚡ Collection frequency (minimum interval)</label>
                    <select id="minIntervalMs">
                        <option value="0">Full amount, tick by tick (highest frequency, about 1~10ms)</option>
                        <option value="10">10ms</option>
                        <option value="50">50ms</option>
                        <option value="100">100ms</option>
                        <option value="250">250ms</option>
                        <option value="500" selected>500ms (recommended)</option>
                        <option value="1000">1 second</option>
                        <option value="2000">2 seconds</option>
                        <option value="5000">5 seconds</option>
                    </select>
                    <small style="color:#888;font-size:11px;margin-top:3px">Throttle: save one every X milliseconds, 0 = full write (large file)</small>
                </div>
            </div>
            <hr>
            <div class="config-title" style="font-size:13px;margin-bottom:10px">🎯 Specify the collection currency (empty = automatically collect the currency with the highest rate)</div>
            <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                ...
                <button class="btn-sm btn-info" onclick="addSelectedSymbol()">➕ Add</button>
                <button class="btn-sm" onclick="clearSelectedSymbols()">🗑 Clear</button>
            </div>
            <div class="symbol-tags" id="selectedSymbolTags"></div>
            <hr>
            <div class="button-group" style="margin-top:6px">
                <button class="btn-success" onclick="startFundingMonitor()" id="fundingStartBtn">🚀 Start tariff monitoring</button>
                <button class="btn-danger"  onclick="stopFundingMonitor()"  id="fundingStopBtn" disabled>⏹️Stop monitoring</button>
                <button class="btn-info"    onclick="refreshFundingRates()">🔄 Refresh tariff rates immediately</button>
            </div>
        </div>
        ...
        <div class="countdown-bar" id="countdownBar" style="display:none">
            <span>⏰ Until next settlement:</span>
            <span class="countdown-num" id="countdownNum">--:--:--</span>
            <span id="countdownPhase" class="phase-idle">Standby</span>
        ...
        <div class="funding-dash" id="fundingDash">
            <div class="fd-card">
                <div class="fd-card-title">Monitor status</div>
                <div class="fd-card-value" id="fd-status">Not started</div>
            </div>
            <div class="fd-card">
                <div class="fd-card-title">Collection phase</div>
                <div class="fd-card-value phase-idle" id="fd-phase">idle</div>
            </div>
            <div class="fd-card">
                <div class="fd-card-title">active collector</div>
                <div class="fd-card-value" id="fd-active">0</div>
            </div>
            <div class="fd-card">
                <div class="fd-card-title">last refresh</div>
        ...
        <!-- 资费率排行 -->
        <div class="config-panel">
            <div class="config-title">📊 Tariff rate ranking (in descending order of absolute value, low liquidity small coins have been filtered)<span style="font-size:12px;color:#888;font-weight:400">— Negative = short to long (benefit from going long), positive = pay long to short (benefit from shorting) | Empty selection = automatically collect TOP N</span></div>
            <div id="fundingTableArea">
                <div class="loading" style="padding:20px;color:#aaa">Click "Refresh Rate Rate Now" to load data</div>
            </div>
        </div>

        <!-- 活跃采集信息 -->
        <div class="config-panel" id="activeCollectPanel" style="display:none">
            <div class="config-title">📡 Currently collecting automatically<span style="font-size:11px;font-weight:400;color:#667eea;margin-left:10px">🔵 The first coin activates the "Real-time Collection Tab" main collector. The data is complete and the K-line can be viewed in real time | The remaining coins are stored in separate files</span>
            </div>
        ...
        <!-- 日志 -->
        <div class="config-panel">
            <div class="config-title">📜Collect logs</div>
            <div class="log-box" id="collectLog">💤 On standby | 11 minutes before settlement → Pull the rate lock list | 10 minutes before settlement → Start collection | Activate the real-time tab of the first coin to view the K-line in real time</div>
        </div>
    </div>
    ...
    <div id="tab-history" class="tab-content">
        <div class="config-panel">
            <div class="config-title">📂 History file browser</div>
            <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:12px">
                <input type="text" id="historyFolder" value="/opt/trading/app/data/basic" placeholder="文件夹路径" style="flex:2">
                <button class="btn-info btn-sm" onclick="loadFileList()">🔍 Browse</button>
                <button class="btn-sm" onclick="document.getElementById('historyFolder').value='data/funding';loadFileList()">📂 View tariff data</button>
            </div>
            <div id="fileListArea"><div class="loading" style="padding:20px;color:#aaa">Please enter the folder path and click Browse</div></div>
        </div>
        ...
        <div class="chart-controls" id="historyChartControls" style="display:none">
            <div class="config-grid">
                ...
                <div class="control-group">
                    <label>&nbsp;</label>
                    <button class="btn-success btn-sm" onclick="renderHistoryChart()">📊 Redraw</button>
                </div>
            </div>
            <div class="trade-tools-panel">
                <div class="trade-tools-title">🧪 Trading backtesting tool (line prediction)</div>
                <div class="config-grid">
                    <div class="control-group">
                    ...
                    <div class="control-group">
                        <label>Position amount (USDT)</label>
                        <input type="number" id="tradePositionSize" value="100" min="1" step="1">
                    </div>
                    <div class="control-group">
                        <label>&nbsp;</label>
                        <button class="btn-info btn-sm" onclick="applyHistoryTradeBacktest()">📐 Draw line backtest</button>
                    </div>
                    <div class="control-group">
                        <label>&nbsp;</label>
                        <button class="btn-warn btn-sm" onclick="startDragAreaBacktest()">🖱️ Area dragging backtest</button>
                    </div>
                    <div class="control-group">
                        <label>&nbsp;</label>
                        <button class="btn-sm" onclick="clearHistoryTradeBacktest()">🧹 Clear backtest line</button>
                    </div>
                </div>
                <div style="font-size:12px;color:#667; margin-top:6px;">Area drag and drop backtest: After clicking the button, drag a rectangular area on the chart (left boundary intersection = entry, right boundary intersection = exit, upper and lower boundaries = take profit/stop loss).</div>
                <div style="font-size:12px;color:#667; margin-top:4px;">After generating a backtest, you can directly drag the entry/take profit/stop loss lines on the chart, and the system will automatically update the profit and profit-loss ratio.</div>
                <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:8px;">
                    <label style="display:flex;align-items:center;gap:6px;font-size:12px;color:#445;">
                        <input type="checkbox" id="zoomToSelection" checked>Automatically zoom into the selection after dragging</label>
                    <button class="btn-sm" onclick="resetHistoryChartZoom()">🔎 Reset zoom</button>
                </div>
                <div class="funding-panel disabled" id="fundingPanel">
                    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:8px;">
                        <label style="display:flex;align-items:center;gap:6px;font-size:12px;color:#34425f;font-weight:700;">
                            <input type="checkbox" id="enableFundingRate" onchange="toggleFundingInputs()">Enable funding rate settlement</label>
                    </div>
                    <div class="config-grid" style="margin-bottom:0;">
                        ...
                        <div class="control-group">
                            <label>Number of settlements (manual)</label>
                            <input type="number" id="fundingSettleCount" min="0" step="1" value="1" disabled>
                        </div>
                    </div>
                    <div class="funding-note">Direction description: A positive rate (+) is "long to pay short"; a negative rate (-) is "short to pay long". The settlement time can be filled in for a single judgment, or directly specified manually with the number of settlements.</div>
                </div>
